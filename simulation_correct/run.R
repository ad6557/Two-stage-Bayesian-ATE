...
mean(results$BPSAcoverage,na.rm = TRUE)
mean(results$PSAcoverage,na.rm = TRUE)
mean(results$trueATE-results$BayesATE,na.rm = TRUE)
mean(results$trueATE-results$FreqATE,na.rm = TRUE)

save(results,file=paste0("M",M,"p",p,"_interaction_flatprior.RData"))
