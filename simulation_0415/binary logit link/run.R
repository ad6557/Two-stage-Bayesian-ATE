...
for(m in 1:M){
  ...
  results_NLR[m,12:16]=bayes.quantile.output(total_NLR,log(OR_NLR))
  results_LR[m,12:16]=bayes.quantile.output(total_LR,log(OR_LR))
  results_all[m,12:16]=bayes.quantile.output(total_all,log(OR_all))
  
  results_NLR[m,17:21]=bayes.output(BAY_ATE[,1],BAY_SE[,1],K,log(OR_NLR))
  results_LR[m,17:21]=bayes.output(BAY_ATE[,2],BAY_SE[,2],K,log(OR_LR))
  results_all[m,17:21]=bayes.output(BAY_ATE[,3],BAY_SE[,3],K,log(OR_all))
